using System.Globalization;
using Avalonia;
using Avalonia.Controls;
using Avalonia.Media;
using Avalonia.Media.Imaging;
using Path = Avalonia.Controls.Shapes.Path;

namespace EasyCaptcha.Avalonia;

public class ECaptchaControl : Control
{
    private static readonly Random Random = new();
    private const string EncodedNoiseImage = "";
    private readonly Image _noiseImage;
    private bool _needRender = true;
        
    public ECaptchaControl()
    {
        _noiseImage ??= GetNoiseImage();
    }

    public enum LetterOptionEnum
    {
        Number,
        Alphabet,
        Alphanumeric,
    }
    
    public LetterOptionEnum LetterOption = LetterOptionEnum.Alphanumeric;

    private uint _numberOfLetters = 5;

    public uint NumberOfLetters
    {
        get => _numberOfLetters;
        set
        {
            _numberOfLetters = value switch
            {
                > 8 => 8,
                < 3 => 2,
                _ => value
            };
        }
    }
    
    public sealed override void Render(DrawingContext context)
    {
        if (_needRender)
            CreateCaptcha(context, LetterOption, NumberOfLetters);
        base.Render(context);
    }

    private string _captchaText = string.Empty;

    public string CaptchaText
    {
        get => _captchaText;
    }

    public void Generate()
    {
        _needRender = true;
        InvalidateVisual();
    }

    private void CreateCaptcha(DrawingContext context, LetterOptionEnum letterOption, uint numberOfLetters)
    {
        if (numberOfLetters <= 0) return;

        var captchaText = string.Empty;
        
        context.DrawImage(_noiseImage.Source!, new Rect(Bounds.Size));

        for (var i = 0; i < numberOfLetters; i++)
        {
            var text = GetRandomLetter(letterOption);
            captchaText += text;

            var typeface = new Typeface(new FontFamily("Arial"));
            var formattedText = new FormattedText(GetRandomLetter(letterOption),  new CultureInfo(""), FlowDirection.LeftToRight,
                typeface, GetRandomFontSize(), Brushes.Blue );
            
            var matrix = Matrix.CreateRotation(GetRandomAngle());
            
            var path = GetRandomPath((int)Width, (int)Height);
            
            var geometry = formattedText.BuildGeometry(new Point(i * 40 + path.Data.Bounds.X, 0));
            geometry!.Transform = new MatrixTransform(matrix);

            context.DrawGeometry(GetRandomColorBrush(), null, geometry);
        }
        
        _captchaText = captchaText;
        _needRender = false;
    }

    private SolidColorBrush GetRandomColorBrush()
    {
        var red = (byte)Random.Next(256);
        var green = (byte)Random.Next(256);
        var blue = (byte)Random.Next(256);
        
        var color = Color.FromRgb(red, green, blue);
        
        return new SolidColorBrush(color);
    }

    private static string GetRandomLetter(LetterOptionEnum letterOption)
    {
        var text = letterOption switch
        {
            LetterOptionEnum.Alphabet => GetRandomAlphabet(),
            LetterOptionEnum.Number => GetRandomNumber(),
            LetterOptionEnum.Alphanumeric => GetRandomAlphanumeric(),
            _ => "A"
        };
    
        return text;
    }
    
    private static string GetRandomNumber()
    {
        return Random.Next(0, 9).ToString();
    }
    
    private static string GetRandomAlphabet()
    {
        var charactersAvailable = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".ToCharArray();
        return charactersAvailable[Random.Next(0, charactersAvailable.Length-1)].ToString();
    }
    
    private static string GetRandomAlphanumeric()
    {
        var charactersAvailable = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".ToCharArray();
        return charactersAvailable[Random.Next(0, charactersAvailable.Length - 1)].ToString();
    }

    private static int GetRandomFontSize()
    {
        return Random.Next(15, 30);
    }
    
    private static double GetRandomAngle()
    {
        return Math.PI / 180 * Random.Next(0, 10);
    }
    
    private static Image GetNoiseImage()
    {
        var binary = Convert.FromBase64String(EncodedNoiseImage);
        var bitmapImage = new Bitmap(new MemoryStream(binary));
        return new Image {Source = bitmapImage, Opacity = 0.8, Stretch = Stretch.Fill};
    }
    
    private static Path GetRandomPath(int maxWidth, int maxHeight)
    {
        var pathFigure = new PathFigure { StartPoint = new Point(Random.Next(0, maxWidth / 10), Random.Next(0, maxHeight)) };
        var pointCollection = new List<Point>()
        {
            new(Random.Next(maxWidth / 10, maxWidth * 2 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 2 / 10, maxWidth * 3 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 3 / 10, maxWidth * 4 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 4 / 10, maxWidth * 5 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 5 / 10, maxWidth * 6 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 6 / 10, maxWidth * 7 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 7 / 10, maxWidth * 8 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 8 / 10, maxWidth * 9 / 10), Random.Next(0, maxHeight)),
            new(Random.Next(maxWidth * 9 / 10, maxWidth), Random.Next(0, maxHeight))
        };
        
        var bezierSegment = new PolyLineSegment() { Points = pointCollection };
        var pathSegmentCollection = new PathSegments{ bezierSegment };
        pathFigure.Segments = pathSegmentCollection;
        var pathFigureCollection = new PathFigures { pathFigure };
        var pathGeometry = new PathGeometry { Figures = pathFigureCollection };
        var path = new Path { Stroke = Brushes.Black, StrokeThickness = 1, Data = pathGeometry };
    
        return path;
    }
}