...
public class Capthca
{
    ...
    public void GenerateNew(LetterOptionEnum letterOption, uint numberOfLetters)
    {
        ...
        for (var i = 0; i < numberOfLetters; i++)
        {
            ...
            var formattedText = new FormattedText(text,  new CultureInfo(""), FlowDirection.LeftToRight,
                typeface, GetRandomFontSize(), Brushes.Blue );
            
            _letters.Add(new Letter()
            {
                BrushText = GetRandomColorBrush(),
                FormattedText = formattedText,
                Matrix = Matrix.CreateRotation(GetRandomAngle())
            });
        }
    ...
    public void DrawCaptcha(DrawingContext context, Rect bounds)
    {
        context.DrawImage(_noiseImage?.Source!, new Rect(bounds.Size));
        foreach (var letter in _letters)
        {
            var width = bounds.Width / _numberOfLetters;
            var height = bounds.Height < 50 ? 0 : bounds.Height / 10;
            var geometry = letter.FormattedText.BuildGeometry(new Point(_letters.IndexOf(letter) * width + 10, height));
            
            geometry!.Transform = new MatrixTransform(letter.Matrix);
            
            context.DrawGeometry(letter.BrushText, null, geometry);
        }
    }
